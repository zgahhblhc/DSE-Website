 ...
 *  load balancing.
 */
var BaseURL = document.baseURI.substring(0, document.baseURI.lastIndexOf("/") + 1);
...
function Statistic(endpoint) {
  var httpRequest = new XMLHttpRequest();
  httpRequest.open("GET", BaseURL + "statistic.php?dse=" + endpoint, false);
...
$('nav .button').click(function(){
  ...
  Statistic(this.dataset.storage);
...
// Load viewer count
void function LoadStatistic(){
  var httpRequest = new XMLHttpRequest();
  var placeStat = function (idName, stat) {
    document.getElementById(idName).innerText = stat
  }
  httpRequest.open("GET", BaseURL + "statistic.php?"
    + "_get[]=dse=index&_get[]=dse=home"
    + "&_get[]=dse=people&_get[]=dse=publications"
    + "&_get[]=dse=teachings&_get[]=dse=openings", false
  );
  httpRequest.onload = function(e){
    if (httpRequest.readyState === 4 && httpRequest.status === 200) {
      var stats = JSON.parse(httpRequest.responseText)[0]['content'];
      placeStat('cntIDX', parseInt(stats[0]) + parseInt(stats[1]));
      placeStat('cntPPL', parseInt(stats[2]));
      placeStat('cntPUB', parseInt(stats[3]));
      placeStat('cntTEA', parseInt(stats[4]));
      placeStat('cntOPN', parseInt(stats[5]));
    }
    else
      console.error(httpRequest.statusText);
  }
